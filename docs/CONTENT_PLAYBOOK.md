...
- ❌ **본문에 컨텐츠 이미지 0개** — 가이드 글마다 최소 1장의 **컨텐츠 이미지** (마케팅 자산·일러스트·다이어그램 중 무엇이든) 를 본문 어딘가에 두기. 위치는 본문 후반부 또는 `</main>` 직전 fallback:
  ```html
  ...
  - 삽입 위치 fallback 순서: `</main>` → `<footer` → `</body>` (10편 중 1편은 `</main>` 없는 legacy template)
