...
- ❌ **콘텐츠 수정 후 날짜 미갱신** — 작성일(datePublished)과 수정일(dateModified)을 명확히 구분 + 수정일은 매 수정마다 갱신
  - **datePublished (작성일)** — 페이지가 처음 publish 된 날짜. **절대 변경 금지** (변경 시 SEO 손해, Google·Naver 가 "rewriting" 으로 의심)
  - **dateModified (수정일)** — 마지막 수정 날짜. **모든 수정 commit 마다 갱신**
  - 모든 수정 시 다음 영역 일괄 갱신 (dateModified 만):
    1. JSON-LD `"dateModified": "YYYY-MM-DDT00:00:00+09:00"`
    2. `<meta property="article:modified_time" content="YYYY-MM-DDT00:00:00+09:00">`
    3. visible `<time datetime="YYYY-MM-DD" itemprop="dateModified">...</time>`
    4. hero 영역의 사람-읽기용 날짜 — **반드시 두 날짜 모두 표시** (예: `<time itemprop="datePublished">YYYY-MM-DD</time> 작성 · <time itemprop="dateModified">YYYY-MM-DD</time> 수정`)
  - **단일 visible date 만 표시할 경우** dateModified 를 표시하고 "마지막 업데이트" 라벨 명시 (예: `📅 마지막 업데이트 YYYY-MM-DD`). 그래야 사용자가 "작성일이 5/13" 으로 오해하지 않음
