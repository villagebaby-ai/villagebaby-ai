...
- ❌ **이미지 native aspect ratio 왜곡** — CSS 또는 HTML attribute 로 강제로 가로/세로 비율 변경하지 말 것. 원본 이미지의 native ratio 그대로 표시. 구체적으로:
  ...
  - **CSS `aspect-ratio` + `width:100%` 패턴 시 `height:auto` 필수** — 예: `.hero-deco{aspect-ratio:1/1;width:100%}` 만 쓰면 부모 컨테이너 height 에 끌려가 squash. 반드시 `height:auto` 같이 명시 (2026-05-13 hero 이미지 squash 버그 원인)
...
- **이미지 SEO·GEO 체크리스트 (v1.2.7)** — 다이어그램·인포그래픽 배포 시 다음 7개 반드시 점검:
  1. **alt** — 자연어 1문장, 8~20단어. 페이지 핵심 키워드 포함하되 stuffing 금지. 예: `alt="4세대→5세대 실손 갈아타기 의사결정 5단계 다이어그램. ① 청구 이력 발급 → ② 5세대 손익 시뮬 → ③ 건강 상태 점검 → ④ 재가입 시점 확인 → ⑤ 신규 가입 후 4세대 해지"`
  2. **figcaption** — 1~2문장 친근체. 다이어그램의 핵심 사실을 본문 콘텍스트와 별개로 풀어 적기 (AI 검색은 figcaption 을 본문 텍스트로 추출)
  3. **JSON-LD `Article.image`** — 단일 객체가 아닌 **배열**로 hero + diagram 둘 다 등록. 각 ImageObject 에 `url`/`width`/`height`/`caption`. 대표 이미지에 `"representativeOfPage":true`
  4. **JSON-LD `HowTo`·`FAQPage` 등 schema 노드에도 `image` 필드** 추가 — 해당 schema 가 시각화된 다이어그램이면 URL 등록 (예: HowTo 의 절차를 시각화한 인포그래픽)
  5. **sitemap `<image:image>`** — `<image:loc>` + `<image:title>` + `<image:caption>` 3개 모두. caption 은 figcaption 텍스트 재사용 (XML escape 필수)
  6. **다이어그램 내 워터마크 — 텍스트 X, 옅은 그래픽 로고 OK (v1.2.8)** — **텍스트 워터마크(도메인 문자열, 회사명 텍스트)는 지양** (alt·OCR·GEO 추출 시 노이즈로 작용). 단 **그래픽 로고는 사용자 명시 요청·브랜드 정책 시 opacity 0.15~0.20 좌하단 50px 마진** 으로 적용 OK — 텍스트가 아니라 이미지라 GEO 영향 미미하고 출처·브랜드 인식 강화. sharp 합성 예: 로고 alpha 채널을 raw 픽셀로 `data[i*4+3] *= 0.18`
  7. **표시 폭은 컨테이너의 60~75%** — 예: `780px` container 면 `max-width:520~580px;margin:auto`. 스크롤 부담 감소 + 인라인 정보 흐름 유지. 모바일은 `width:100%` 자동
  - 보조: 파일명도 SEO 신호. `img-{핵심 키워드}.{ext}` 패턴 (한글 percent-encoding OK). `IMG_001.png` 같은 무의미 파일명 금지
...
- 2026-05-14: v1.2.6 — CSS squash 가이드 보강: `.hero-deco` 등 `aspect-ratio + width:100%` 패턴에 `height:auto` 누락하면 부모 height 끌림으로 squash. 항상 `height:auto` 동반 명시
- 2026-05-14: v1.2.7 — 이미지 SEO·GEO 7체크: alt 자연어 1문장 8~20단어 / figcaption 1~2문장 / Article.image 배열로 hero+diagram / HowTo·FAQ 등 schema 에도 image / sitemap image:loc+title+caption / 다이어그램 워터마크 텍스트 지양 / 표시 폭 컨테이너 60~75%
- 2026-05-14: v1.2.8 — 워터마크 가이드 명확화: **텍스트** 워터마크는 지양(OCR·GEO 노이즈), **그래픽 로고**는 사용자 요청·브랜드 정책 시 opacity 0.15~0.20 좌하단 OK. 추가: 단순 색감 다이어그램은 WebP 가 PNG 보다 클 수 있음 → `webp({lossless:true})` 시도 또는 PNG main 선택
- (이후 갱신은 여기 추가)
