...
- ❌ **명사형 종결** (예: "22주 전까지 가능. 부담보 위험.") → **친근체** ("대부분의 보험사는 22주 전까지 받아줘요. 다만 진단을 받은 뒤라면 부담보 조건이 붙을 수 있어요.")
- ❌ **영어 단독 표기** (예: "Baby blues", "PPD") → **한국어 + 부연 설명** ("산후 우울감", "산후우울증"). 의학 약어가 꼭 필요하면 `한국어 (영어 약어)` 형태
- ❌ **FAQ 답변 명사형** → 모든 FAQ 답변은 친근체 평어 "~해요/~예요" 종결. JSON-LD `FAQPage` 의 answer text 도 본문과 1:1 동기화 필수
- ❌ **다이어그램 desc·caption 명사형** → 마찬가지로 친근체. AI 검색이 이 텍스트를 본문으로 추출하므로 더 중요
...
- ❌ **이미지 native aspect ratio 왜곡** — CSS 또는 HTML attribute 로 강제로 가로/세로 비율 변경하지 말 것. 원본 이미지의 native ratio 그대로 표시. 구체적으로:
  - `<img>` 의 `width` / `height` attribute 는 **반드시 native dim 과 동일한 값** (예: native 192×410 이면 `width="192" height="410"`)
  - 둘 중 하나만 변경하지 말 것 (예: `width="100" height="100"` 으로 squash → 인물 찌그러짐)
  - CSS 에서 한 축만 지정해 비율 유지하려면 다른 축은 `auto` (예: `width:160px; height:auto`)
  - 부득이 정사각 frame 등에 넣어야 하면 `object-fit:contain` (이미지 비율 유지 + 여백) 또는 `object-fit:cover` (frame 채우며 일부 잘림). `object-fit:fill` 절대 사용 금지 (왜곡 발생)
  - 1:1 / 16:9 등 특정 ratio 강제 ≠ 원칙. **원본 그대로** 가 원칙
- ❌ **콘텐츠 수정 후 날짜 미갱신** — 작성일과 수정일을 명확히 구분
  - **datePublished (작성일)** — JSON-LD 에만 유지. **절대 변경 금지** (변경 시 SEO 손해)
  - **dateModified (수정일)** — 모든 수정 commit 마다 갱신
  - 모든 수정 시 일괄 갱신 (dateModified 만):
    1. JSON-LD `"dateModified": "YYYY-MM-DDT00:00:00+09:00"`
    2. `<meta property="article:modified_time" content="YYYY-MM-DDT00:00:00+09:00">` (있다면)
    3. hero 영역의 visible date — **dateModified 만 표시**: `최종 업데이트 <time datetime="YYYY-MM-DD" itemprop="dateModified">YYYY-MM-DD</time>`
  - **visible 영역에 datePublished 표시 금지** — 작성일이 오래되면 "stale 콘텐츠" 인상. 수정일만 보여줘서 "최신성" 강조
  - **"수정" 보다 "업데이트" 라는 단어 권장** — 친숙하고 캐주얼한 어조

- ❌ **글자수·분량으로 hero meta 표기** (예: "5천 자", "9천 자 종합", "1만 자") → **읽기 소요 시간 (분)** 으로 표기
  - 형식: `⏱ 약 N분 소요`
  - 계산식: 본문 글자수 (공백 제외) ÷ 300자/분 × 1.15 (표·다이어그램 보정), **5분 단위 반올림** (최소 5분)
  - 글자수는 사용자에게 의미가 약함. "N분" 이 더 행동 결정에 도움 (지금 읽을지 나중에 읽을지)

- ❌ **hero meta 영역 표준 형식 불일치** → 다음 4 요소 순서·형식 통일:
  ```html
  <div class="meta">
    <span>📅 최종 업데이트 <time datetime="YYYY-MM-DD" itemprop="dateModified">YYYY-MM-DD</time></span>
    <span>👥 작성: <strong><a href="https://babybilly.co" target="_blank" rel="author noopener" style="color:inherit;text-decoration:none;border-bottom:1px dotted currentColor">베이비빌리</a> 보험 전문팀</strong></span>
    <span>⏱ 약 N분 소요</span>
    <span>🔗 <a href="/카테고리/pillar-슬러그/">{pillar 글} →</a></span>
  </div>
  ```
  - **datePublished 는 visible 영역에 표시 X** (JSON-LD only). 사용자에게는 "최종 업데이트" 만 노출
  - "수정" 보다 "업데이트" 단어 사용
  - "{N}분 소요" 형식 (분 단위, "약 N분 소요")
- ❌ **SVG 다이어그램 font-size < 13** → 모바일에서 가독성 ↓. 최소 `font-size="13"` 이상. 더 작아야 한다면 wrapper 에 `@media(max-width:760px){.diagram-figure{overflow-x:auto}.diagram-figure svg{min-width:520px}}` 추가
- ❌ **표가 모바일에서 잘림** → `@media(max-width:760px){.sec table{overflow-x:auto;display:block;max-width:100%}}` 필수

- ❌ **`.hero-deco` (또는 hero 장식 이미지) 의 CSS 에 `height` 미명시** — 일부 브라우저가 HTML attribute `height` 값을 강제 적용해 squash 발생. CSS 에서 한 축만 지정해 비율 유지하려면 **`.hero-deco { ... ; height: auto }` 명시 필수**

- ❌ **og:image 모든 페이지 공통 1개 (`heroimage.jpg` 등)** — 공유 시 단조로움 + 페이지별 차별화 신호 약함
  - 페이지마다 자동 생성된 카드형 og:image 필수
  - 사이즈: 1200×630 (og 표준)
  - 위치: `/assets/og/{slug}.png` + `.webp` (둘 다 export)
  - 카드 구성: 카테고리 뱃지 + 글 제목 + 부제 + (작성일·읽기시간) + favicon + brand tagline
  - 자동 생성: SVG 템플릿 → sharp `.png()` / `.webp()` export
  - **메타 8종 일괄 적용**:
    ```html
    <meta property="og:image" content="https://villagebaby.kr/assets/og/{slug}.png">
    <meta property="og:image:alt" content="{글 제목} — {부제} | 베이비빌리">
    <meta property="og:image:width" content="1200">
    <meta property="og:image:height" content="630">
    <meta property="og:image:type" content="image/png">
    <meta property="og:site_name" content="베이비빌리">
    <meta property="og:locale" content="ko_KR">
    <meta name="twitter:image:alt" content="{글 제목} — {부제} | 베이비빌리">
    ```
  - sitemap `<image:image><image:loc>...</image:loc><image:title>...</image:title></image:image>` 도 같이 등록 (Google Images 인덱싱)

- ❌ **다이어그램을 inline `<svg>` 로만 두기** — Google 이미지 검색에 거의 인덱싱 안 됨. 외부 사이트 우클릭 저장도 불가. 다이어그램 = **PNG/WebP `<picture>` 패턴** 권장:
  ```html
  <figure class="diagram-figure">
    <picture>
      <source srcset="/assets/diagrams/{name}.webp" type="image/webp">
      <img src="/assets/diagrams/{name}.png" alt="{풍부한 설명 — 다이어그램이 전달하는 핵심 정보 모두 포함}" width="W" height="H" loading="lazy">
    </picture>
    <figcaption>{사용자에게 친근체로 풀어 쓴 해설}</figcaption>
  </figure>
  ```
  - 이미지 파일 위치: `/assets/diagrams/{name}.png` + `.webp` 둘 다
  - **sitemap.xml 의 해당 페이지 `<url>` 안에 `<image:image><image:loc>...</image:loc><image:title>...</image:title></image:image>` 등록 필수** (이미지 검색 강한 신호)
  - 디자인 자체는 SVG 로 그려도 됨 → sharp 의 `sharp(svgBuffer, {density: 192}).png()` / `.webp()` 로 export
  - alt 와 figcaption 에 다이어그램이 전달하는 모든 핵심 정보를 텍스트로 풀어 적기 (AI 검색 봇이 텍스트 추출). inline SVG `<title>`/`<desc>` 의 텍스트 추출 효과를 대체

- ❌ **본문에 컨텐츠 이미지 0개** — 가이드 글마다 최소 1장의 **컨텐츠 이미지** (마케팅 자산·일러스트·다이어그램 중 무엇이든) 를 본문 어딘가에 두기. **위치 우선순위**: (1) TOC `</nav>` 직후 — 도입부 topic 시각화 / (2) TOC 없으면 author-card `</aside>` 직후 / (3) hub/home 은 hero `</section>` 직후 / (4) 모두 없을 때만 `</main>` 직전 fallback. **글 맨 아래(footer 직전) 배치는 시각적으로 어색해서 금지**.
  ```html
  <figure class="content-image" style="margin:48px auto 16px;max-width:560px;text-align:center">
    <picture>
      <source srcset="/assets/content/{cat}/{slug}.webp" type="image/webp">
      <img src="/assets/content/{cat}/{slug}.jpg"
           alt="{글 제목} — {부제 또는 desc} | 베이비빌리"
           title="{글 제목}"
           width="1200" height="1200"
           loading="lazy" decoding="async"
           style="width:100%;max-width:560px;height:auto;border-radius:14px;display:block;margin:0 auto;box-shadow:0 4px 16px rgba(0,0,0,0.08)">
    </picture>
    <figcaption style="font-size:13.5px;color:#6B7280;margin-top:12px;line-height:1.6">{글 제목} — {부제 또는 desc}</figcaption>
  </figure>
  ```
  - 파일 위치 표준: `/assets/content/{category}/{slug}.{jpg,webp}` (예: `/assets/content/guide/태아보험-완전정리.jpg`)
  - **JPG + WebP 두 포맷 모두 업로드** (sharp 로 1200×1200 → 1080×1080 inside fit, quality 82, mozjpeg). 평균 페이지당 jpg ~45KB + webp ~22KB
  - **alt 와 figcaption 은 서로 달라야 함**. alt 는 SEO/접근성용 (`{title} — {desc} | 베이비빌리`), figcaption 은 visible 컨텍스트 (`{title} — {desc}`). "베이비빌리 X 가이드 가이드" 같은 **중복 단어 금지** (특히 title 끝이 "가이드"인 경우)
  - sitemap.xml 의 해당 `<url>` 에 `<image:image><image:loc>` 도 등록
  - 모바일에서는 max-width 가 화면 폭에 맞춰 자동 축소. PC 에서는 560px 최대 (1200px 원본의 약 47%) — 본문 흐름 안에 자연스럽게 배치되는 사이즈

- ❌ **OG 카드 일러스트 매칭 — title 전체(부제 포함) 로 매칭하지 말 것**. 부제에 "암"/"병"/"치매" 같은 의료 키워드가 들어가면 첫 번째 regex 가 무조건 매칭돼서 모든 카드가 `baby-medical` 로 override 됨 (실측: cancer/female 4종 잘못 매칭). **mainTitle (부제 제외) 만 입력**. 또한 우선순위는 (1) `완전정리` pillar 룰 → 카테고리 default 가 최상위 / (2) `가족력|비교|분석` → baby-magnifier / (3) `임신|태아` → baby-angel / (4) `산후|우울` → baby-wink / ... 순. 의료 키워드는 9번째 우선순위로 — pillar/검색/임신 보다 항상 아래
...
- 2026-05-13: v1.2 — Section 13 (금지 사항) 에 4종 추가: (1) 명사형 종결·영어 단독 표기·FAQ 답변 (2) 이미지 1:1 ratio 원칙 (3) 수정 시 4곳 날짜 자동 갱신 (4) SVG/table 모바일 가독성
- 2026-05-13: v1.2.1 — 이미지 비율 가이드 정정: "1:1 강제" → "native aspect ratio 유지" 가 원칙. `object-fit:fill` 금지, `width/height` attribute 는 native dim 그대로
- 2026-05-13: v1.2.2 — 날짜 가이드 정정: datePublished 절대 변경 금지 / dateModified 만 갱신. visible 영역에 두 날짜 모두 표시 권장 (단일 표시 시 "마지막 업데이트" 라벨 명시)
- 2026-05-13: v1.2.3 — (1) visible 에는 dateModified 만, datePublished 는 JSON-LD only. "최종 업데이트" 라벨 + "업데이트" 단어 사용. (2) hero meta 의 글자수 → "약 N분 소요" 로 표기 (300자/분 × 1.15, 5분 단위 반올림)
- 2026-05-13: v1.2.4 — 다이어그램 가이드 추가: inline SVG 대신 PNG/WebP `<picture>` + sitemap `<image:loc>` 등록 (Google 이미지 검색 인덱싱 강화). alt·figcaption 에 텍스트 풀어 적기 (AI 검색 추출)
- 2026-05-13: v1.2.5 — hero meta 표준 형식 명시 (📅 최종 업데이트 · 👥 작성 · ⏱ N분 소요 · 🔗 pillar). datePublished 는 JSON-LD only, visible 에 표시 금지. 글자수→분 단위 표기 강제
- 2026-05-13: v1.2.6 — (1) `.hero-deco { height: auto }` 명시 필수 추가 — HTML attribute height 강제 squash 방지 (2) og:image 카드 페이지별 자동 생성 + 메타 8종 표준 + sitemap image:loc 등록 가이드 추가
- 2026-05-21: v1.2.7 — 본문 컨텐츠 이미지 표준 추가 (Section 13). `/assets/content/{cat}/{slug}.{jpg,webp}` 위치 + `<figure class="content-image">` + alt/figcaption 분리 (중복 단어 금지) + 삽입 위치 fallback (`</main>` → `<footer` → `</body>`) + sitemap image:image 등록. 37편에 일괄 적용 (commits `d6dbe57f` / `9876f14f` / `3e5198c4` / `2605e19e`)
- 2026-05-21: v1.2.8 — (1) 컨텐츠 이미지 **위치 우선순위 정정**: TOC `</nav>` 직후 > author-card `</aside>` 직후 > hero `</section>` 직후 > `</main>` fallback. 글 맨 아래 배치 금지 (a8fc4326 / 64d63118 / e29d1ffa). (2) OG 카드 `pickIllust()` 룰 정정: mainTitle 만 입력 + pillar/검색/임신 우선순위가 의료 키워드보다 높아야 함 (cancer/female 4종 잘못 매칭 사례 발견 후 수정)
- 2026-05-21: v1.2.9 — (1) **og:image 메타태그 일괄 fix**: 156편 중 92편이 옛날 `heroimage.jpg` 가리키는 버그 발견 → 8종 메타(image·alt·width·height·type·site_name·locale·twitter:image·twitter:image:alt) 전면 적용 (918d849b). (2) **본문 이미지 100% 커버리지**: 드롭박스 마케팅 자산 없는 119편에도 `/assets/og/{slug}.png` (OG 카드, 1200×630) 를 본문 안 TOC 직후에 삽입. fallback chain 확장: `<nav class="toc">` > `<aside class="author-card">` > `<section id="hero">` > `<section class="pillar-hero">` > `<div class="hero">` > `</header>` > `</h1>` (81be20d9 / ac97c118). sitemap image:image 도 119편 일괄 등록
- 2026-05-21: v1.2.10 — A·D·C 일괄 정리 작업. (A) 본문 `<li>` 명사형 종결 672개 117편 → 친근체 자동 변환 (필요/가능/중요/권장/필수/적합/추천/보장/확인/점검/검토/비교/선택/면제/등록/가입/허용/금지/의무/제외 26 규칙) (abfa5cbc). (D) 156편 SEO 9체크리스트 audit (title·desc·h1·canonical·og 8종·JSON-LD·breadcrumb·content-image·internal links≥3) → 90/156 만점 + 56편 8/9. 4개 critical fix: og:image 누락 2편 (9ce363d3), home canonical 누락 1편, 내부 링크 <3 인 2편 (244867d6). 27편 magazine canonical 은 percent-encoded 라 valid (false positive). (C) SEO 내부 라벨 `(Pillar)`/`(Spoke)`/`(Hub)`/`(Core)` 사용자 노출 6편 제거 (bc2dd85b)
- 2026-05-21: v1.2.11 — (1) **사이트 전체 404 일괄 fix**: 7종 dead URL 30+ 페이지 referer 정리. `/compare/` → `/guide/태아보험-보험사-비교-매트릭스/`, `/senior/` → `/care/`, `/about/` `/contact/` → `https://babybilly.co/`, `/guide/태아보험-가입시기/` → `/guide/태아보험-가입시기-필수특약/`, `/guide/태아보험-특약/` → `/guide/태아보험-특약-총정리/`, `/guide/유산방지주사-태아보험/` → `/guide/태아보험-거절-사례/#step02`, magazine NT 대소문자 정정. 70+개 링크 28편 + 잔여 3편 추가 fix. **0 broken internal links** 달성 (a5d5547c / 2ba7dd3c / ea0d0a7e). (2) **admin dashboard 재생성**: 73개 → 157편 100% sitemap sync. 20개 섹션 (매거진 5종 신설: pregnancy/newborn/toddler/school/case, 도구·시스템 분리) (f8f895c8)
- (이후 갱신은 여기 추가)
